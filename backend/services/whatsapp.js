...
function normalizePhone(phone) {
  ...
  // Uruguay: si viene como 93405195, lo convertimos a 59893405195
...
function getBookingValue(booking, ...keys) {
  for (const key of keys) {
    if (booking && booking[key] !== undefined && booking[key] !== null && booking[key] !== "") {
      return booking[key];
    }
  }
...
async function sendBookingConfirmationMessage(booking = {}, professional = {}) {
  ...
  const businessName =
    ...
    professional?.name ||
    getBookingValue(booking, "business_name", "businessName", "professional_name", "professionalName") ||
    "TuAgendaYa";

  const serviceName =
    getBookingValue(booking, "service_name", "serviceName") ||
    "Servicio";
  ...
  const headerText = "Confirmación de reserva";
  ...
  const confirmId = token
    ...
    : `confirm_booking_id:${booking.id || booking.bookingId || ""}`;

  const cancelId = token
    ? `cancel_booking:${token}`
    : `cancel_booking_id:${booking.id || booking.bookingId || ""}`;
...
// Alias para que bookings.js funcione aunque importe cualquiera de los dos nombres.
