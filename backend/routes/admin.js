...
async function deleteProfessionalCompletelyByEmail(email) {
  ...
  try {
    ...
    if (!professional) {
      ...
      return {
        ok: false,
        status: 404,
        body: { error: "Profesional no encontrado", email: normalizedEmail },
      };
    }
    ...
    if (client) await client.query("ROLLBACK");
    console.error("deleteProfessionalCompletelyByEmail error:", error);
    return {
      ok: false,
      status: 500,
      body: {
...
// TEMPORAL navegador: borrar julieta con clave.
// Quitar después de usar.
router.get("/test/delete-professional-browser", async (req, res) => {
  ...
  if (email !== "julieta@gmail.com") {
    return res.status(400).json({
      error: "Por seguridad este endpoint temporal solo borra julieta@gmail.com",
    });
  }

  const result = await deleteProfessionalCompletelyByEmail(email);
