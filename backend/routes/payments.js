...
router.get("/test/disable-promo", async (req, res) => {
  try {
    const key = String(req.query.key || "");
    const expectedKey = process.env.TEMP_TEST_KEY || "tuagendaya_test_2026";
    ...
    await db.query(`
      ...
      ADD COLUMN IF NOT EXISTS promo_started_at TIMESTAMP
    `);
    ...
    const result = await db.query(
      `
        UPDATE professionals
        SET
          promo_started_at = NOW() - INTERVAL '5 months',
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({
        ok: false,
        error: "No se encontró profesional con ese email.",
        email,
      });
    }
    ...
    return res.json({
      ok: true,
      message: "Promo desactivada para prueba de Mercado Pago.",
      professional: result.rows[0],
      nextStep: "Entrá al perfil profesional y probá Pagar automático.",
    });
  } catch (error) {
    console.error("Error disabling test promo:", error);
    return res.status(500).json({
      ok: false,
      error: "No se pudo desactivar la promo de prueba.",
    });
  }
});
